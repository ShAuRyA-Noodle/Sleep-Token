...
| # | Feature | Where |
|---|---------|-------|
| 1 | **Karpathy-style autonomous research loop** — `program.md` driven, fixed-budget, bootstrap-CI95 accept/reject | `ShAuRyA_Supplymind/autoresearch/` |
| 2 | **Live geopolitical ingestion** — 5 real data sources, SQLite store | `ShAuRyA_Supplymind/realtime/` |
| 3 | **Real crisis library** — 8 Iran/Israel/Hormuz 2024-2026 events with 26 citations | `ShAuRyA_Supplymind/scenarios/` |
| 4 | **Fixed SPOF detector** — F1 0.949 → 1.000 on 3 real graphs | `ShAuRyA_Supplymind/features/spof_v2.py` |
| 5 | **Proper stacking framework** — OOF + meta-learner on DataCo | `ShAuRyA_Supplymind/features/stacking_v2.py` |
| 6 | **Reproducibility receipts** — every headline number gets `.receipt` + `.reproduce.sh` | `ShAuRyA_Supplymind/receipts/` |
| 7 | **GCN attention viz** — edge betweenness + flow importance | `ShAuRyA_Supplymind/features/gcn_attention_viz.py` |
| 8 | **Counterfactual explainer** — what-if loss projection with analog lookup | `ShAuRyA_Supplymind/features/counterfactual_explainer.py` |
| 9 | **Pareto carbon slider** — cost/resilience/CO₂ multi-objective | `ShAuRyA_Supplymind/features/pareto_carbon.py` |
| 10 | **RAG provenance graph** — citation trust tiers | `ShAuRyA_Supplymind/features/rag_provenance.py` |
| 11 | **Conformal-calibrated RL** — split-conformal intervals on Q-values | `ShAuRyA_Supplymind/features/conformal_rl.py` |
| 12 | **Gradio leaderboard** — external submission harness | `ShAuRyA_Supplymind/features/leaderboard.py` |
| 13 | **Qwen-VL port imagery** — 7-port satellite assessment | `ShAuRyA_Supplymind/features/qwen_vl_port_imagery.py` |
| 14 | **Multi-agent competition** — Apple/Samsung/Toyota chip-shortage dynamic | `ShAuRyA_Supplymind/features/multi_agent_demo.py` |
| 15 | **DT risk-appetite slider** — conservative/balanced/aggressive surrogate | `ShAuRyA_Supplymind/features/dt_risk_slider.py` |
| 16 | **CUDA kernel verify** — PyTorch fallback benchmark (0.034ms at B=1024) | `ShAuRyA_Supplymind/features/cuda_kernel_verify.py` |
| 17 | **LoRA training harness** — supplymind-analyst v5 with real rubric data | `ShAuRyA_Supplymind/features/lora_train.py` |
| 18 | **Modelfile v5** — 8 calibrated few-shots + A/B benchmark | `ShAuRyA_Supplymind/features/Modelfile.analyst_v5` |
| 19 | **Arxiv-style preprint** — consolidated 1-page technical summary | `ShAuRyA_Supplymind/docs/PREPRINT.md` |
| 20 | **External-outreach playbook** | `ShAuRyA_Supplymind/docs/EXTERNAL_OUTREACH.md` |
...
- **NewsAPI rate-limited**: the crisis library (`ShAuRyA_Supplymind/scenarios/iran_israel_hormuz_2024_2026.json`) has 8 hand-curated events with 26 citations, so the demo works offline.
