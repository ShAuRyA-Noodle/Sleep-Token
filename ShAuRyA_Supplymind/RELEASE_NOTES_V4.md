...
v3.0-arcadia is **frozen** at `02251e9`. v4.0-arcadia-live is a **purely additive** release that lives in `ShAuRyA_Supplymind/` and mounts into `server/app.py` via a single include_router line.

Every v3 test still passes. Every v3 number is unchanged. v4 adds:

| Class | What | Evidence |
|-------|------|----------|
| **Karpathy autoresearch** | Agent-driven `candidate_train.py` mutation + fixed-budget CI95 accept/reject | `ShAuRyA_Supplymind/autoresearch/` (9 files) |
| **Live geopolitical pipeline** | Real-time NewsAPI+GDELT+USGS+FRED+MarineTraffic ingestion → `/live/hormuz-closure` endpoint | `ShAuRyA_Supplymind/realtime/` (7 files) + endpoint |
| **Real crisis library** | 8 Iran/Israel/Hormuz 2024-2026 events with 26 citations | `ShAuRyA_Supplymind/scenarios/iran_israel_hormuz_2024_2026.json` |
| **15 unique features F1-F10 + G-fixes** | Fully tested, committed | `ShAuRyA_Supplymind/features/` (17 modules) |
| **Reproducibility receipts** | Every headline number is one bash command | `ShAuRyA_Supplymind/receipts/` (13 receipts) |
...
| **v4 tests (`ShAuRyA_Supplymind/tests/`)** | — | **76 new** |
| **Total** | **173** | **249** |
| Skipped | 0 | 0 |
...
| **G2 HF deploy** | v2 | v4-ready: guide + smoke checklist | `ShAuRyA_Supplymind/deploy/HF_DEPLOY_V4.md` |
...
5. `pytest tests/ ShAuRyA_Supplymind/tests/ -q` — 249 green
