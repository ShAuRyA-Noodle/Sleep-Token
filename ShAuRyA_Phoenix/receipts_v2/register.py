...
To regenerate a single receipt:
    ...
    python -m ShAuRyA_Phoenix.receipts_v2.register --regenerate --only R5_GRANITE_mxbai_P1

To just emit the YAML+sh files without running (useful for committing stubs
before we have the environment ready):

    python -m ShAuRyA_Phoenix.receipts_v2.register --stub
"""
...
# -----------------------------------------------------------------------------
# v4 carryovers (13 receipts, grade-A upgrade of the existing ShAuRyA_Supplymind/receipts/)
# -----------------------------------------------------------------------------
...
V4_CARRYOVERS = [
    ...
    Receipt(
        ...
        command="python -m ShAuRyA_Supplymind.features.spof_v2 --eval-all --out /tmp/spof.json",
    ...
    Receipt(
        ...
        command="python -m ShAuRyA_Supplymind.features.stacking_v2 --out /tmp/stack.json",
    ...
    Receipt(
        ...
        command="python -m ShAuRyA_Supplymind.realtime.sources.fred_brent --latest-only",
    ...
    Receipt(
        ...
        command="pytest tests/ ShAuRyA_Supplymind/tests/ -q --tb=no",
...
V5_NEW = [
    Receipt(
        ...
        command="python -m ShAuRyA_Phoenix.autoresearch_fixed.rebuild_state",
        extraction='python -c "import json; s=json.load(open(r\\"ShAuRyA_Phoenix/autoresearch_fixed/state.json\\")); print(s[\\"best\\"][\\"experiment_name\\"] if s[\\"best\\"] else \\"\\")"',
    ...
    Receipt(
        ...
        command="python -m ShAuRyA_Phoenix.autoresearch_fixed.rebuild_state",
        extraction='python -c "import json; s=json.load(open(r\\"ShAuRyA_Phoenix/autoresearch_fixed/state.json\\")); h=[x for x in s[\\"history\\"] if x[\\"experiment_name\\"]==\\"s3_curriculum_learning\\"][0]; print(h[\\"delta_ci95_lower\\"])"',
        expected="0.05",
        comparator=">=",
    ),
    Receipt(
        ...
        command="python -m ShAuRyA_Phoenix.arena.leaderboard",
        extraction='python -c "import json; b=json.load(open(r\\"ShAuRyA_Phoenix/experiments/arena/leaderboard.json\\")); print(b[\\"n_baselines\\"], b[\\"rows\\"][0][\\"policy_name\\"])"',
    ...
    Receipt(
        ...
        command='python -m ShAuRyA_Phoenix.counterfactual_twin.twin --severity 0.85 --brent 123 --rollouts 20 --task easy_typhoon_response --out ShAuRyA_Phoenix/experiments/twin/V5_receipt_run.json',
        extraction='python -c "import json; print(json.load(open(r\\"ShAuRyA_Phoenix/experiments/twin/V5_receipt_run.json\\", encoding=\\"utf-8\\")).get(\\"savings_vs_no_action_usd\\"))"',
        expected="0",
        comparator=">=",
    ),
    Receipt(
        ...
        command="python -m ShAuRyA_Phoenix.roll_integration.dpo_judge.prepare_preference_data",
        extraction='wc -l < ShAuRyA_Phoenix/roll_integration/dpo_judge/data/preference_pairs.jsonl',
        expected="20",
        comparator=">=",
    ),
    Receipt(
        ...
        command="ls ShAuRyA_Phoenix/supplymind_skills/*/SKILL.md ShAuRyA_Phoenix/supplymind_skills/plugin.json",
        extraction="ls ShAuRyA_Phoenix/supplymind_skills/*/SKILL.md ShAuRyA_Phoenix/supplymind_skills/plugin.json | wc -l",
        expected="4",
        comparator=">=",
    ),
    Receipt(
        ...
        command="pytest ShAuRyA_Phoenix/tests/ -q --tb=no",
        extraction='grep -oE "[0-9]+ passed" || true',
        expected="passed",
        comparator="regex",
