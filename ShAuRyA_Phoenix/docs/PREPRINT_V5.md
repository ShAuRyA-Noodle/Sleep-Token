...
v5 adds `FORCE_REPLAY=1` flag + frozen cache (`ShAuRyA_Phoenix/realtime_v5/
...
`ShAuRyA_Phoenix/roll_integration/` contains four submodules:
...
`ShAuRyA_Phoenix/supplymind_skills/` — 3 Claude Code skills:
...
`ShAuRyA_Phoenix/receipts_v2/` — 20 receipts, each a YAML + `reproduce.sh`
...
`python -m ShAuRyA_Phoenix.receipts_v2.register --regenerate` re-runs every
...
Draft PR descriptions at `ShAuRyA_Phoenix/upstream_prs/{meta_openenv,
...
```bash
pandoc ShAuRyA_Phoenix/docs/PREPRINT_V5.md -o preprint_v5.pdf --pdf-engine=xelatex
```
