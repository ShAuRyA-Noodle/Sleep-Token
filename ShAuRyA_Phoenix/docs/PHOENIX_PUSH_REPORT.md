...
Location: `ShAuRyA_Phoenix/.venv-roll/` (isolated from main `.venv`).

```bash
$ ShAuRyA_Phoenix/.venv-roll/Scripts/python.exe -c "import roll; from trl import DPOTrainer; print('ok')"
ok
```

Installed: `roll` (editable, `--no-deps` against `ROLL-main/ROLL-main`), `trl 0.9.6`, `transformers 4.56+`, `peft`, `accelerate`, `datasets`, `bitsandbytes`, `httpx`, `pyyaml`, `rich`.
...
`ShAuRyA_Phoenix/autoresearch_fixed/state.json` rebuilt with real data from both v4 original runs + Phoenix reruns:
...
`ShAuRyA_Phoenix/roll_integration/dpo_judge/data/preference_pairs.jsonl`

21 real pairs (each {prompt, chosen, rejected, meta}) derived from:
- `v3_arcadia/results/R4_DANGEROUS_V2.json` → 26 scenarios with hand-labeled ground truth
...
gh pr create --repo meta-pytorch/openenv \
  ...
  --body-file ~/Desktop/Sleep-Token/ShAuRyA_Phoenix/upstream_prs/meta_openenv/PR.md
```
...
gh pr create --repo alibaba/ROLL \
  ...
  --body-file ~/Desktop/Sleep-Token/ShAuRyA_Phoenix/upstream_prs/alibaba_roll/PR.md
```
...
ShAuRyA_Phoenix/.venv-roll/Scripts/pip.exe install "trl>=0.11,<0.13" --upgrade
bash ShAuRyA_Phoenix/experiments/dpo_judge_v1/train_dpo.sh

# B. Downgrade transformers (works but drags other deps)
ShAuRyA_Phoenix/.venv-roll/Scripts/pip.exe install "transformers==4.42.4" --force-reinstall
bash ShAuRyA_Phoenix/experiments/dpo_judge_v1/train_dpo.sh
...
2. **`pytest ShAuRyA_Phoenix/tests/ -q`** → confirms 16/16 green. 3 seconds.
