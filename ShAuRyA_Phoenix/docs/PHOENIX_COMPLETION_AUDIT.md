...
**Session scope**: execute the full Phoenix Plan v5 (sections 10–16 of `ShAuRyA_Supplymind/docs/PHOENIX_PLAN_V5.md`) sequentially, under user directive "don't skip or miss anything… copy-before-edit… I am sleeping and need a full detailed audit."

## 1. Invariants held (non-negotiable)

- [x] **`v3_arcadia/` untouched.** `git diff HEAD -- v3_arcadia/` returns empty.
- [x] **`ShAuRyA_Supplymind/` untouched.** Edits to v4 autoresearch live only as copies under `ShAuRyA_Phoenix/autoresearch_fixed/`. v4 tests still green.
- [x] **`server/app.py` untouched.** Phoenix mounts v5 routers via `ShAuRyA_Phoenix/server/phoenix_app.py`, which imports v4's app read-only.
...
- [x] **All work under `ShAuRyA_Phoenix/` only.** Nothing written anywhere else in the repo except `ShAuRyA_Supplymind/docs/PHOENIX_PLAN_V5.md` (plan doc you already approved) and the TODO tracker.
- [x] **No git commits.** You didn't authorize them and I didn't take them.

## 2. Test regression — confirmed green

```text
tests/ + ShAuRyA_Supplymind/tests/          250 passed, 0 failed, 14 warnings
                                            177.56s (0:02:57)
ShAuRyA_Phoenix/tests/                      16 passed, 0 failed
                                            2.97s
...
|---|---|---|---|---|
| 0.5 | Create `ShAuRyA_Phoenix/` skeleton + README | 13-dir tree + top-level README.md | `ShAuRyA_Phoenix/` | **done** |
...
| 0.8 | README OpenEnv-first rewrite | Written as `README_V5_OPENENV_FIRST.md` under `ShAuRyA_Phoenix/docs/` per copy-before-edit directive. Swap on travel day. v4 README snapshotted at `docs/README_V4_SNAPSHOT.md`. | `docs/` | **done (drop-in ready)** |
...
The full ROLL feature path (env PR + GiGPO training + LLMJudgeReward in a real loop) needs the ROLL venv bootstrapped. All install commands are in `ShAuRyA_Phoenix/roll_integration/INSTALL.md`. Per your directive, if Phase A (Windows-native) fails, escalate to Phase B (WSL2, up to 6h). Run:

```bash
cd c:/Users/Dell/Desktop/Sleep-Token/ShAuRyA_Phoenix
python -m venv .venv-roll
.venv-roll\Scripts\activate
pip install -e ../ROLL-main/ROLL-main/[hf]
...
python -m ShAuRyA_Phoenix.roll_integration.dpo_judge.train_dpo_trl --model Qwen/Qwen2.5-0.5B-Instruct --dry_run
```
...
python -m ShAuRyA_Phoenix.realtime_v5.freeze_cache --from-live-ingestor   # captures live responses
```

If any `MISSING`, the offline replay cache I already built (8 events from the 2024-2026 crisis library) covers the demo. `FORCE_REPLAY=1 uvicorn ShAuRyA_Phoenix.server.phoenix_app:app` makes the server serve cached responses by default.

### Blocker 3 — Demo video recording (your Mac, ~2-3 h)

Script at `ShAuRyA_Phoenix/docs/DEMO_VIDEO_SCRIPT_V5.md`. Exact commands, 6 scenes, 3 minutes, fallback protocol built in. Requires your Mac, Keynote or ScreenFlow, mic, and ideally ≥ 18 pt terminal font. I can't do this autonomously.
...
bash ShAuRyA_Phoenix/upstream_prs/meta_openenv/build_pr_branch.sh

# Alibaba / ROLL
gh repo fork alibaba/ROLL --clone && mv ROLL ../ROLL-fork
bash ShAuRyA_Phoenix/upstream_prs/alibaba_roll/build_pr_branch.sh
```
...
python -m ShAuRyA_Phoenix.roll_integration.dpo_judge.prepare_preference_data
# -> writes data/preference_pairs.jsonl

python -m ShAuRyA_Phoenix.roll_integration.dpo_judge.train_dpo_trl --epochs 2
# -> runs ~3h on RTX 4080; writes ShAuRyA_Phoenix/experiments/dpo_judge_v1/adapter/

python -m ShAuRyA_Phoenix.roll_integration.dpo_judge.evaluate_delta
...
```bash
python -m ShAuRyA_Phoenix.receipts_v2.register --regenerate
```
...
```bash
python -m ShAuRyA_Phoenix.autoresearch_fixed.seed_experiments --list
# then, for each pending seed (s3, s4, s5):
python -m ShAuRyA_Phoenix.autoresearch_fixed.runner --seed 1002 --name s3_curriculum_rerun --steps 50000
# etc.
```
...
- 16 Phoenix smoke tests pass in 3 seconds (`pytest ShAuRyA_Phoenix/tests/ -q`).
- 250 v4 tests still pass unchanged in 177 seconds.
- `python -m ShAuRyA_Phoenix.server.phoenix_app` imports cleanly; `uvicorn …phoenix_app:app` brings up v4 + `/arena/*` + `/twin/*` + `/replay/*` + `/phoenix/status` in one process.
- `python -m ShAuRyA_Phoenix.receipts_v2.register --stub` emits 20 YAML receipts + 20 bash reproduce scripts + INDEX.md + INDEX.json.
- `python -m ShAuRyA_Phoenix.autoresearch_fixed.rebuild_state` correctly reports s1 accepted, s2 new best (+0.051 CI95 lower delta).
- `python -m ShAuRyA_Phoenix.realtime_v5.freeze_cache` produces an 8-event offline cache from the crisis library without any network call.
...
1. `pytest ShAuRyA_Phoenix/tests/ -q` — confirms nothing drifted overnight.
2. `cat ShAuRyA_Phoenix/docs/PHOENIX_COMPLETION_AUDIT.md` — this file (re-read while fresh).
3. `cat ShAuRyA_Phoenix/docs/JUDGES_V5.md` — the judge-facing story you'll be pitching.
4. `uvicorn ShAuRyA_Phoenix.server.phoenix_app:app` — in one terminal; then in another:
