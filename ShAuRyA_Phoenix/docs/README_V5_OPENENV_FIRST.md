...
bash ShAuRyA_Phoenix/receipts_v2/R5_GRANITE_mxbai_P1.reproduce.sh
# -> expected 0.9622
```
...
- Upstream PR draft (`ShAuRyA_Phoenix/upstream_prs/meta_openenv/`) submitting
...
Total: 20 receipts live in `ShAuRyA_Phoenix/receipts_v2/`. Run any:

```bash
bash ShAuRyA_Phoenix/receipts_v2/<claim_id>.reproduce.sh
```
...
10. **Phoenix server entrypoint** — `uvicorn ShAuRyA_Phoenix.server.phoenix_app:app` mounts v4 + v5 routers in one process.
...
*Full technical details: `ShAuRyA_Phoenix/docs/PREPRINT_V5.md`. Reproducibility
receipts: `ShAuRyA_Phoenix/receipts_v2/INDEX.md`. Judge path: `docs/JUDGES_V5.md`.*
