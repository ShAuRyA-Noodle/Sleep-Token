...
Two modes:
    ...
    1. --from-crisis-library (default, offline-safe)
        ...
        ShAuRyA_Supplymind/scenarios/iran_israel_hormuz_2024_2026.json and
    ...
    2. --from-live-ingestor (requires NewsAPI/FRED/GDELT keys)
        Calls ShAuRyA_Supplymind.realtime.ingestor with --once and captures
        the output. Produces the most realistic cache but only works today
        when the APIs answer.

Output: ShAuRyA_Phoenix/realtime_v5/replay_cache_<timestamp>.json
...
LIVE_CRISES = ROOT / "ShAuRyA_Supplymind" / "scenarios" / "iran_israel_hormuz_2024_2026.json"
...
def build_from_crisis_library() -> dict:
    ...
    return {
        "schema_version": "1.0",
        "source": "ShAuRyA_Supplymind/scenarios/iran_israel_hormuz_2024_2026.json",
...
def build_from_live_ingestor() -> dict:
    ...
    try:
        proc = subprocess.run(
            [sys.executable, "-m", "ShAuRyA_Supplymind.realtime.ingestor", "--once",
    ...
    return {
        "schema_version": "1.0",
        "source": "ShAuRyA_Supplymind.realtime.ingestor --once",
