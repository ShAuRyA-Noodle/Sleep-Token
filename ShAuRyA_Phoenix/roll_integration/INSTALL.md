...
python -m ShAuRyA_Phoenix.roll_integration.dpo_judge.train_dpo_trl \
...
| Ray install hanging | Ray is only needed for multi-node. Skip: `pip install -e ..\ROLL-main\ROLL-main\[hf] --no-deps` then install deps manually. |
...
```bash
cd /mnt/c/Users/Dell/Desktop/Sleep-Token/ShAuRyA_Phoenix
...
`ShAuRyA_Phoenix/receipts_v2/V5_ROLL_install_phase.reproduce.sh` showing
