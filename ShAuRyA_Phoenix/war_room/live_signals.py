...
ShAuRyA_Phoenix/realtime_v5/freeze_cache.py. The fallback path is visibly
...
def fetch_news_headlines(query: str, max_results: int = 5) -> list[dict]:
    ...
    if not os.environ.get("NEWS_API_KEY"):
        ...
        return []
    try:
        from ShAuRyA_Supplymind.realtime.sources.newsapi import fetch_recent
...
def fetch_brent_usd() -> tuple[float | None, dict]:
    ...
    try:
        from ShAuRyA_Supplymind.realtime.sources.fred_brent import fetch_latest_brent
...
def load_replay_fallback() -> list[dict]:
    ...
    cache_path = ROOT / "ShAuRyA_Phoenix" / "realtime_v5" / "replay_cache_latest.json"
    ...
    for ev_id, ev in blob.get("events", {}).items():
        out.append({
            ...
            "url": "",
            "source": "ShAuRyA_Phoenix replay_cache",
