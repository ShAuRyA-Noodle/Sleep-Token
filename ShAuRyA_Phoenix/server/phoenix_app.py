...
Imports v4's `server.app:app` (frozen — no edits), then mounts:
    /arena     (ShAuRyA_Phoenix.arena.router)
    /twin      (ShAuRyA_Phoenix.counterfactual_twin.router)
    /replay    (ShAuRyA_Phoenix.realtime_v5.replay_adapter)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("ShAuRyA_Phoenix.server.phoenix_app:app", host="0.0.0.0", port=8000, reload=False)
