...
def _safe_predict(model: Any, obs: np.ndarray, action_masks) -> int:
    """Call model.predict; swallow action_masks if unsupported; robust to
    RecurrentPPO's `(action, state)` tuple and to batched (shape>1) returns
    that .item() rejects. Phoenix v5 fix for v4 s4_recurrent_ppo crash."""
    try:
    ...
    if arr.size == 0:
        raise ValueError("empty action from model.predict")
    return int(arr[0])
