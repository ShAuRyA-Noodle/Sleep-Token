...
def _s3_curriculum(old: str) -> str:
    """Curriculum via save/reload (v5 fix): set_env mid-training breaks
    MaskablePPO's cached action_dims; instead save→load with the new env.
    Identical training math, no crash."""
    new_block = '''
...
def train_policy(model, env, total_steps: int) -> None:
    """Curriculum 40/30/30 via save→reload instead of set_env()."""
    import tempfile, os
    from sb3_contrib import MaskablePPO
    ...
    # Stage 2: medium — save weights, rebuild on new env
    with tempfile.TemporaryDirectory() as tmp:
        ckpt = os.path.join(tmp, "stage1.zip")
        model.save(ckpt)
        ...
        # Stage 3: hard
        ckpt2 = os.path.join(tmp, "stage2.zip")
        model.save(ckpt2)
        env3 = _curriculum_env("hard")
        model = MaskablePPO.load(ckpt2, env=env3, device=model.device)
        model.learn(total_timesteps=budget_hard, progress_bar=False, reset_num_timesteps=False)


def architecture_summary() -> str:
    return "MaskablePPO [128,128] curriculum easy->med->hard via save/reload (40/30/30 split)"
